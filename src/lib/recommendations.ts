...
export type Category = 'structure' | 'code' | 'query' | 'seoForAi' | 'freshness';
...
function freshnessRecommendations(scores: ScoreResult, crawl: CrawlResult): Recommendation[] {
  ...
  if (crawl.errors.length > 0) {
    ...
    recs.push({
      category: 'freshness',
      priority: crawl.errors.length > 5 ? 'high' : 'medium',
...
export function generateRecommendations(scores: ScoreResult, crawl: CrawlResult): Recommendation[] {
  ...
  // Always generate recommendations for low-scoring categories (< 15)
  // but also include critical ones regardless of score
  ...
  const filtered = allRecs.filter((rec) => {
    ...
    return categoryScore < 15 || rec.priority === 'high';
  });
